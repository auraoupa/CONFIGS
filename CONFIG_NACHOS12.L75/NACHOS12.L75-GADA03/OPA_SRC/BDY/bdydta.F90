...
!$AGRIF_END_DO_NOT_TREAT
   ...
   !! $Id: bdydta.F90 6140 2015-12-21 11:35:23Z timgraham $ 
...
!!AA debug
                   IF(lwp) WRITE(numout,*) 'bdydta good case'
                   IF(lwp) WRITE(numout,*) 'size of array hti : ', SIZE(bf(jfld_hti)%fnow(:,1,1))
                   IF(lwp) WRITE(numout,*) 'size of array hts : ', SIZE(bf(jfld_hts)%fnow(:,1,1))
                   IF(lwp) WRITE(numout,*) 'size of array ai : ', SIZE(bf(jfld_ai)%fnow(:,1,1))
!! fin AA debug
   ...
   SUBROUTINE bdy_dta_init
      ...
      ! AA debug
      IF(lwp) WRITE(numout,*) 'bdydta nb_bdy_fld_sum = ',nb_bdy_fld_sum
      ! fin AA debug
...
902         IF( ios /= 0 )   CALL ctl_nam ( ios , 'nambdy_dta in configuration namelist', lwp )
            ...
            nblen => idx_bdy(ib_bdy)%nblen
            !AA debug
             IF(lwp) WRITE(numout,*) 'bdydta nblen =',nblen
            ! fin debug AA
            nblenrim => idx_bdy(ib_bdy)%nblenrim
            dta => dta_bdy(ib_bdy)
            dta%nread(2) = 0
            ...
            IF( nn_dyn2d_dta(ib_bdy) == 1 .or. nn_dyn2d_dta(ib_bdy) == 3 ) THEN 
               ...
               IF( dta%ll_ssh ) THEN 
                  ...
                  !AA debug
                  IF(lwp) WRITE(numout,*) 'bdydta ib_bdy =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta bn_ssh =',bn_ssh
                  IF(lwp) WRITE(numout,*) 'bdydta jfld =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta igrid(jfld) =',igrid(jfld)
                  IF(lwp) WRITE(numout,*) 'bdydta ilen1%ll_ssh =',ilen1(jfld)
                  ! fin debug AA
                  ilen3(jfld) = 1
           ...
           &  ( ln_full_vel_array(ib_bdy) .AND. ( nn_dyn2d_dta(ib_bdy) == 1 .or. nn_dyn2d_dta(ib_bdy) == 3 ) ) ) THEN
               ...
               IF( dta%ll_u3d .OR. ( ln_full_vel_array(ib_bdy) .and. dta%ll_u2d ) ) THEN
                  ...
                  !AA debug
                  IF(lwp) WRITE(numout,*) 'bdydta ib_bdy =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta bn_u3d =',bn_u3d
                  IF(lwp) WRITE(numout,*) 'bdydta jfld =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta igrid(jfld) =',igrid(jfld)
                  IF(lwp) WRITE(numout,*) 'bdydta ilen1%ll_u3d =',ilen1(jfld)
                  ! fin debug AA
                  ilen3(jfld) = jpk
               ...
               IF( dta%ll_v3d .OR. ( ln_full_vel_array(ib_bdy) .and. dta%ll_v2d ) ) THEN
                  ...
                  !AA debug
                  IF(lwp) WRITE(numout,*) 'bdydta ib_bdy =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta bn_v3d =',bn_v3d
                  IF(lwp) WRITE(numout,*) 'bdydta jfld =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta igrid(jfld) =',igrid(jfld)
                  IF(lwp) WRITE(numout,*) 'bdydta ilen1%ll_v3d =',ilen1(jfld)
                  ! fin debug AA
                  ilen3(jfld) = jpk
            ...
            IF( nn_tra_dta(ib_bdy) == 1 ) THEN
               ...
               IF( dta%ll_tem ) THEN
                  ...
                  !AA debug
                  IF(lwp) WRITE(numout,*) 'bdydta ib_bdy =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta bn_tem =',bn_tem
                  IF(lwp) WRITE(numout,*) 'bdydta jfld =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta igrid(jfld) =',igrid(jfld)
                  IF(lwp) WRITE(numout,*) 'bdydta ilen1%ll_temp =',ilen1(jfld)
                  ! fin debug AA
                  ilen3(jfld) = jpk
               ENDIF
               ...
               IF( dta%ll_sal ) THEN
                  ...
                  !AA debug
                  IF(lwp) WRITE(numout,*) 'bdydta ib_bdy =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta bn_sal =',bn_sal
                  IF(lwp) WRITE(numout,*) 'bdydta jfld =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta igrid(jfld) =',igrid(jfld)
                  IF(lwp) WRITE(numout,*) 'bdydta ilen1%ll_sal =',ilen1(jfld)
                  ! fin debug AA
                  ilen3(jfld) = jpk
               ENDIF
            ...
            IF( nn_ice_lim_dta(ib_bdy) == 1 ) THEN
               ...
               IF( dta%ll_a_i ) THEN
                  ...
                  !AA debug
                  IF(lwp) WRITE(numout,*) 'bdydta ib_bdy =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta bn_a_i =',bn_a_i
                  IF(lwp) WRITE(numout,*) 'bdydta jfld =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta igrid(jfld) =',igrid(jfld)
                  IF(lwp) WRITE(numout,*) 'bdydta ilen1%ll_a_i =',ilen1(jfld)
                  ! fin debug AA
               ...
               IF( dta%ll_ht_i ) THEN
                  ...
                  !AA debug
                  IF(lwp) WRITE(numout,*) 'bdydta ib_bdy =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta bn_ht_i =',bn_ht_i
                  IF(lwp) WRITE(numout,*) 'bdydta jfld =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta igrid(jfld) =',igrid(jfld)
                  IF(lwp) WRITE(numout,*) 'bdydta ilen1%ll_ht_i =',ilen1(jfld)
                  ! fin debug AA
                  IF ( ll_bdylim3 ) THEN ; ilen3(jfld)=jpl ; ELSE ; ilen3(jfld)=1 ; ENDIF
               ENDIF
               ...
               IF( dta%ll_ht_s ) THEN
                  ...
                  !AA debug
                  IF(lwp) WRITE(numout,*) 'bdydta ib_bdy =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta bn_ht_s =',bn_ht_s
                  IF(lwp) WRITE(numout,*) 'bdydta jfld =',ib_bdy
                  IF(lwp) WRITE(numout,*) 'bdydta igrid(jfld) =',igrid(jfld)
                  IF(lwp) WRITE(numout,*) 'bdydta ilen1%ll_ht_s =',ilen1(jfld)
                  ! fin debug AA
                  IF ( ll_bdylim3 ) THEN ; ilen3(jfld)=jpl ; ELSE ; ilen3(jfld)=1 ; ENDIF
               ENDIF
      ...
      DO jfld = 1, nb_bdy_fld_sum
         ! AA debug
         IF(lwp) WRITE(numout,*) 'bdy_dta jfld =',jfld
         IF(lwp) WRITE(numout,*) 'bdy_dta ilen1(jfld) =',ilen1(jfld)
         IF(lwp) WRITE(numout,*) 'bdy_dta ilen3(jfld) =',ilen3(jfld)
         ! fin AA debug
